document.addEventListener('DOMContentLoaded', () => {
  ...
  const bookmarkedNames = JSON.parse(localStorage.getItem("bookmarkedItems")) || [];
  const bookmarkedGadgets = allGadgets.filter(g => bookmarkedNames.includes(g.name));

  if (bookmarkedGadgets.length === 0) {
    favoritesList.innerHTML = "<p>No bookmarks found.</p>";
    return;
  }

  bookmarkedGadgets.forEach(gadget => {
